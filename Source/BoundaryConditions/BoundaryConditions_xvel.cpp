        ...
        ParallelFor(
            bx_xlo, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].lo(0) == ERFBCType::ext_dir) {
                    ...
                    dest_arr(dom_lo.x,j,k) =  (4.0*dest_arr(dom_lo.x+1,j,k)-dest_arr(dom_lo.x+2,j,k))/3.0 ;
        ...
        ParallelFor(
            bx_xhi, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) {
                ...
                if (bc_ptr[n].hi(0) == ERFBCType::ext_dir) {
                    ...
                    //dest_arr(dom_hi.x+1,j,k) = (4.0*dest_arr(dom_hi.x,j,k) - dest_arr(dom_hi.x-1,j,k))/3.0;
					if(j==2 and k == 10){
						std::cout << "Value is " << dom_hi.x << " " << i << " " << dest_arr(dom_hi.x,j,k) << " " << dest_arr(dom_hi.x+1,j,k)  << "\n";
					}

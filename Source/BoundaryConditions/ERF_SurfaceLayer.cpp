...
SurfaceLayer::update_fluxes (const int& lev,
                             ...
                             const Vector<MultiFab>* surface_state_at_lev,
                             int max_iters)
{
    ...
    if (flux_type == FluxCalcType::CUSTOM_HINDCAST) {
        for (amrex::MFIter mfi(*u_star[lev]); mfi.isValid(); ++mfi) {
            ...
            const Array4<const Real>& surf_arr = (*surface_state_at_lev)[0].array(mfi);

            ParallelFor(gtbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                u_star_arr(i, j, 0) = surf_arr(i,j,0);
                t_star_arr(i, j, 0) = surf_arr(i,j,1);
                q_star_arr(i, j, 0) = surf_arr(i,j,2);
                //printf("ustar is %0.15g %0.15g %0.15g\n", u_star_arr(i, j, 0), t_star_arr(i,j,0), q_star_arr(i,j,0));
            });
        }
    }

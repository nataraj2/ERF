				  ...
				  const MultiFab& mf_SMark)
{
    ...
    source_terms_cellcentered(geom, cons_in, U_old, V_old, mf_vars_simpleAD);
   ...
   amrex::ParallelAllReduce::Sum(disk_cell_count.data(),
    ...
    for(int it=0; it<xloc.size(); it++){
        std::cout << "turbine index, freestream velocity is " << it << " " << freestream_velocity[it] << " " << 
															   disk_cell_count[it]  <<  " " << freestream_velocity[it]/(disk_cell_count[it] + 1e-10) << " " << 
															   xloc[it] << " " << yloc[it] <<"\n";
        }
    }
}
    ...
    for ( MFIter mfi(cons_in,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(gbx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            ...
            for(long unsigned int it=0;it<nturbs;it++){
				...
				bool is_cell_marked = find_if_marked(x1, x2, y1, y2, d_xloc_ptr[it], d_yloc_ptr[it],
                                                     nx, ny, d_hub_height, d_rotor_rad, y, z);

				if(is_cell_marked) {	
                	check_int++;
                    source_x = -2.0*std::pow(avg_vel, 2.0)*0.5*(1.0-0.5);
                    source_y = 0.0;
            	}
            }
            if(check_int > 1){

...
void Kessler::AdvanceKessler (const Real time)
{
    ...
    for ( MFIter mfi(*tabs,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(box3d, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
			...
			//if (xloc > 20.0e3 and xloc < 30e3 and zloc > 1.0e3 and zloc < 5.0e3 and qc_array(i,j,k) > 0.001)// and time > 900.0 and time < 1800.0)
			if (qp_array(i,j,k) > 1e-3 and time < 3600.0)// and time > 900.0 and time < 1800.0)
			{	
				Real heating = theta_array(i,j,k)/tabs_array(i,j,k)*5.0/60.0*dtn;
				theta_array(i,j,k) = theta_array(i,j,k) + heating;
				mwave_heating_array(i,j,k) = heating;
			}
			else{
				mwave_heating_array(i,j,k) = 0.0;
			}
